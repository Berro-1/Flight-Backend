...
class Taxi {
    private $mysqli; // This will hold the database connection object.
...
//It take a MySQLi connection object as a parametr and sets it for use in the class
    ...
    public function __construct($mysqli) {
        $this->mysqli = $mysqli; // Store the connection object for later use in the class.
    }
    ...
    public function getById($taxi_id) {
        ...
        $stmt = $this->mysqli->prepare('SELECT * FROM taxis WHERE taxi_id = ?');
        if (!$stmt) {
            return ["error" => "Prepare failed: " . $this->mysqli->error]; // Return error message if failed
        }
        $stmt->bind_param('i', $taxi_id);// to provide sql injection


        if (!$stmt->execute()) {
            return ["error" => "Execute failed: " . $this->mysqli->error]; 
        }
    ...
    public function createTaxi($driver_name, $status, $from_location, $to_location) {
        $stmt = $this->mysqli->prepare('INSERT INTO taxis (driver_name, status, from_location, to_location) VALUES (?, ?, ?, ?)');
    ...
    public function getAllTaxis() {
        $stmt = $this->mysqli->prepare('SELECT * FROM taxis');
        $stmt->execute();
        $result = $stmt->get_result();
    ...
    public function deleteTaxi($taxi_id) {
        $stmt = $this->mysqli->prepare('DELETE FROM taxis WHERE taxi_id=?');
    ...
    public function searchTaxisByDriver($driver_name) {
        $stmt = $this->mysqli->prepare('SELECT * FROM taxis WHERE driver_name LIKE ?');
